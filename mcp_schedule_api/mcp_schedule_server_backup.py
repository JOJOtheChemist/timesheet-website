...
将FastAPI应用转换为Model Context Protocol (MCP)格式
"""
...
class ScheduleItem:
    ...
    def to_dict(self):
        return {
            "id": self.id,
            "schedule_date": self.schedule_date.isoformat() if isinstance(self.schedule_date, date) else self.schedule_date,
...
class MCPScheduleServer:
    def __init__(self):
        self.server_info = {
            "name": "mcp-schedule-api",
            "version": "1.0.0",
            "description": "MCP Schedule API Server for timesheet management"
        }
        self.resources = {}
        self.tools = {}
...
async def main():
    ...
    logger.info("MCP Schedule API Server is ready!")
    logger.info("Server Info:", server.server_info)
