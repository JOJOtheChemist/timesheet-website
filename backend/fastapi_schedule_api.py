...
async def get_user_profile(current_user: Optional[Dict[str, Any]] = Depends(get_current_user), db: mysql.connector.MySQLConnection = Depends(get_db)):
    if not current_user:
        raise HTTPException(status_code=401, detail="未认证")
    try:
        cursor = db.cursor(dictionary=True)
        user_id = current_user.get("sub")
        cursor.execute("SELECT major, goal, projects FROM user_profiles WHERE user_id = %s", (user_id,))
        profile = cursor.fetchone()
        if not profile:
            return {"major": None, "goal": None, "projects": None}
        return {"major": profile.get("major"), "goal": profile.get("goal"), "projects": profile.get("projects")}
    except Exception as e:
...
async def update_user_profile(profile_data: UserProfileUpdate, current_user: Optional[Dict[str, Any]] = Depends(get_current_user), db: mysql.connector.MySQLConnection = Depends(get_db)):
    if not current_user:
        raise HTTPException(status_code=401, detail="未认证")
    try:
        cursor = db.cursor(dictionary=True)
        ...
        if existing_profile:
            ...
            if profile_data.major is not None:
                ...
                update_values.append(profile_data.major)
            if profile_data.goal is not None:
                update_fields.append("goal = %s")
                update_values.append(profile_data.goal)
            if profile_data.projects is not None:
                update_fields.append("projects = %s")
                update_values.append(profile_data.projects)
            
            if update_fields:
                update_values.append(user_id)
                cursor.execute(f"UPDATE user_profiles SET {', '.join(update_fields)}, updated_at = NOW() WHERE user_id = %s", update_values)
        else:
            # 创建新画像
            cursor.execute(
                "INSERT INTO user_profiles (user_id, major, goal, projects) VALUES (%s, %s, %s, %s)",
                (user_id, profile_data.major, profile_data.goal, profile_data.projects)
            )
